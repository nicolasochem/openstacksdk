...
class Proxy(proxy.Proxy):
    ...
    def live_migrate_server(
        ...
        :param block_migration:
            Perform a block live migration destination host by the scheduler.
            Can be 'auto', True or False. Some clouds are too old to support
            'auto', in which case a ValueError will be thrown. If omitted, the
            value will be 'auto' on clouds that support it, and False on clouds
            that do not.
        :returns: None
        """
        server = self._get_resource(_server.Server, server)
